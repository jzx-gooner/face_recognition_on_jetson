...
cv::Mat mergeDiffPic(vector<string> pic_list, int pic_cols_rows, string output_file, bool horizontal, bool draw_rect)
{
    int pic_cols = 0;
    int pic_rows = 0;
    int max_cols=0;
    ...
    for(int i=0; i<pic_num; i++){
        ...
        auto file_name = iLogger::file_name(pic_list[i], false);
        putText(input[i], file_name, cv::Point(0, 40), 0, 1, Scalar(0,255,0), 1, 16);
        //draw_rect为true,画矩形
        if(draw_rect){
